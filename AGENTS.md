...
- `omarchy-hw-arch` / `omarchy-hw-aarch64` / `omarchy-hw-apple-silicon` - architecture and Apple Silicon detection; do not call `uname -m` or grep `/proc/device-tree/compatible` in install or runtime code. `omarchy-mac-setup`, both `omarchy-upgrade-to-quattro*` entrypoints, `omarchy-system-boot-to-esp`, and `omarchy-system-btrfs-migrate` keep self-contained, status-checked architecture guards because they can run as downloaded scripts before a checkout or new helpers exist. The standalone Mac upgrade also keeps its device-tree warning. Historical migrations keep their original checks.
